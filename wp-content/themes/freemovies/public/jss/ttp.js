...
$(document).ready(function() {
    $('.more-info').tooltipster({
        ...
        functionInit: function(instance, helper){
            var content = $(helper.origin).find('.info').detach();
    ...
    if($('.change-view').hasClass('on')) {
        $('.more-infos').tooltipster('disable');
      
    } else {

    	$('.more-infos').tooltipster({
	        side: ['right', 'left'],
	        minWidth: 250,
	        maxWidth: 250,
	        interactive: true,
	        ...
	        animationDuration: 0,
	        functionInit: function(instance, helper){
	               var content = $(helper.origin).find('.info').clone();
	            instance.content(content);
        ...
        $('.more-infos').tooltipster('enable');

    }


    $('body').on('click', '.change-view i', function(event) {
    	event.preventDefault();

    	var $this = $(this).parent();

        if($('#list-view').hasClass('on')){
            $('.change-view').addClass('on');
        } else {
            $('.change-view').removeClass('on');
        }
    	if($('.change-view').hasClass('on')) {
    		console.log('si on');
